...
uint16_t isPrincipalSubscribedService(uint8_t principal, uint8_t service) {
  for (i = 0; i < currentNumSubscriptions; i++) {
    if ((sensorSubscriptions[i].principal == principal) &&
        (service == sensorSubscriptions[i].service))
...
void removeElementSubscription(uint16_t subscriptionID) {
  ...
  for (i = 0; i < currentNumSubscriptions - 1; i++) {
    if (sensorSubscriptions[i].subscriptionID == subscriptionID) {
      ...
      memmove(&sensorSubscriptions[i], &sensorSubscriptions[i + 1],
              currentNumSubscriptions - (i + 1));
      currentNumSubscriptions--;
      return;
    }
  }
  // Handle the last subscription problem
  if (i == currentNumSubscriptions - 1 &&
      sensorSubscriptions[i].subscriptionID == subscriptionID) {
    currentNumSubscriptions--;
  }
}
...
void NotifyApp(uint8_t *payload, uint16_t length) {
  for (uint8_t j = 0; j < currentNumSubscriptions; j++) {
    ...
    if (service == *payload &&
        sensorSubscriptions[j].disabled ==
            false) { // The first byte of the payload is sensor type
      switch (service) {
      case TIMER:
        sensorSubscriptions[j].current_time += 1;
        if (sensorSubscriptions[j].current_time >=
            sensorSubscriptions[j].window_interval) {
          sensorSubscriptions[j].current_time = 0;
          AmuletSensorsEvt *timeReqEvt;
          timeReqEvt = Q_NEW(AmuletSensorsEvt, AMULET_REQ_CHANGE_SIG);
          QACTIVE_POST(getAppActiveObject(GlobalAppQueueHead, subscribedAppID),
                       &timeReqEvt->super, NULL);
        }
        // When timer updates, also update battery status
        // TODO: Is there a better place to do this? (Steven Hearndon 7/4/16)
        CoreRefreshBatteryStatus(false);
        break;
        break;
      case LIGHT:
        break;
      ...
      default:
        if (service == ACCELEROMETER) {
          ...
          dataEvt = Q_NEW(AmuletSensorsEvt, AMULET_DEVICE_DATA_IMU_SIG);
  ...
  for (uint8_t j = 0; j < currentNumSubscriptions; j++) {
    if (*payload == TIMER) {
      ...
      if (sensorSubscriptions[j].expiration <= 0 &&
          sensorSubscriptions[j].continuous == false) {
        if (ACCELEROMETER == sensorSubscriptions[j].service) {
          acc_ref_counter--;
        }
        if (IMU == sensorSubscriptions[j].service) {
          gyro_ref_counter--;
        }
...
bool isBLESensor(uint8_t sensor_id) {
  if (sensor_id == HEARTRATE || sensor_id == RUNNING_SPEED) {
    return true;
  } else
    return false;
}
...
void SubscribeSensor(uint8_t sensor_id, uint16_t expiration,
  ...
  if (!isBLESensor(sensor_id) && sensor_id != TIMER) {
    ...
    if (subscription_already_exist == 0) {
      ...
      // enqueueIPC(sensor_id, expiration, window_size, window_interval,
      // requestor,
  ...
  if (IMU == sensor_id) {
    gyro_ref_counter++;
    // Now turn on the Gyro to normal mode
    GyroNormalMode();
  }
  if (sensor_id == HEARTRATE || sensor_id == RUNNING_SPEED) {
    ble_ref_counter++;
  ...
  // Handle other sensor subscriptions, i.e. start up the sensing duty cycle I
  // guess
...
void UnsubscribeSensor(uint8_t sensor_id, uint8_t requestor) {
  ...
  for (i = 0; i < currentNumSubscriptions; i++) {
    if ((sensorSubscriptions[i].principal == requestor) &&
        (sensor_id == sensorSubscriptions[i].service)) {
...
void CoreTimerCancel(uint8_t requestor) { UnsubscribeSensor(TIMER, requestor); }
